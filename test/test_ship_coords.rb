...
#commenting out old tests; they will have to be rewritten
=begin
  def setup
    # Note the .fleet method: these are arrays of Ship objects.
    header = header_generator("player")
    @board = Board.new(header)
    header = header_generator("enemy")
    @enemy_board = Board.new(header)
    @enemy_fleet = Fleet.new(contender:"enemy", board:@board,
      fleet:@ship_list).fleet
    $testing = true # to avoid "Enter to continue" prompts when testing
    @player_fleet = Fleet.new(contender:"player",
